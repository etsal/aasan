...
static inline int st_memcmp(void __arena *mem1, void __arena *mem2, size_t size)
{
	u8 __arena *bytes1 = (u8 __arena *)mem1;
	u8 __arena *bytes2 = (u8 __arena *)mem2;
	int i;
...
#define ALLOC_OR_FAIL(bytes, alignment) ({ 				\
	...
	if (!mem) {							\
		bpf_printk("%s:%d scx_static_alloc failed", __func__, __LINE__);	\
		scx_static_destroy();					\
		return -ENOMEM;						\
	}								\
	mem;								\
...
do { 										\
	if (scx_static_init(((bytes) >> PAGE_SHIFT))) {				\
		bpf_printk("%s:%d scx_static_init failed", __func__, __LINE__);	\
		return -ENOMEM;							\
	}									\
} while (0)
...
do { 									\
	if (st_isset((mem), (val), (size))) {				\
		bpf_printk("%s:%d val %d missing", __func__, __LINE__);	\
		return -EINVAL;						\
	}								\
} while (0)
...
do { 									\
	if (st_memcmp((mem1), (mem2), (size))) {			\
		bpf_printk("%s:%d regions differ", __func__, __LINE__);	\
		return -EINVAL;						\
	}								\
} while (0)
...
do { 										\
	if ((u64)(mem) & ((alignment) - 1)) {					\
		bpf_printk("%s:%d invalid alignment", __func__, __LINE__);	\
		return -EINVAL;							\
	}									\
} while (0)
 ...
 */
static
int scx_selftest_static_alloc_single(u64 bytes, u64 alignment)
{
	u8 __arena *barray;
	void __arena *mem;
	int i;
...
static
int scx_selftest_static_alloc_multiple(u64 bytes, u64 alignment)
{
...
static
int scx_selftest_static_alloc_aligned(void)
{
	void __arena *mem;
	u64 alignment;
	int round;
	...
	for (round = 0; round < 2 && can_loop; round++) {
		for (alignment = 1; alignment <= PAGE_SIZE && can_loop; alignment <<= 1) {
			mem = ALLOC_OR_FAIL(1, alignment);
			ALIGNED_OR_FAIL(mem, alignment);
		}

		for (alignment = PAGE_SIZE; alignment >= 1 && can_loop; alignment >>= 1) {
			mem = ALLOC_OR_FAIL(1, alignment);
			ALIGNED_OR_FAIL(mem, alignment);
		}
	}
...
static
int scx_selftest_static_alloc_exhaustion(u64 bytes, u64 alignment)
{
...
#define SCX_STATIC_SELFTEST(suffix, ...) SCX_SELFTEST(scx_selftest_static_ ## suffix, __VA_ARGS__)
